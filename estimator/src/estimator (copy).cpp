                                                 ...
                                                 std::vector<GRBVar> &quaternion, std::vector<GRBVar> &scale)
{
  ...
  for (int i = 0; i < WINDOW_SIZE - 1; i++)
  {
...
#ifdef DEBUG_estimator
    // cout << "活跃特征点数 = " << cam1_counter << ", " << cam2_counter << endl; // debug
    ...
    obj += ((10 * scale[2 * i + 1] * velocity_x_2 / cam2_counter + 1e5 * velocity[P_id]) *
                (10 * scale[2 * i + 1] * velocity_x_2 / cam2_counter + 1e5 * velocity[P_id]) +
            (-10 * scale[2 * i] * velocity_x_1 / cam1_counter + velocity[P_id + 1]) *
                (-10 * scale[2 * i] * velocity_x_1 / cam1_counter + 1e5 * velocity[P_id + 1]) +
            (-5 * (scale[2 * i] * velocity_y_1 / cam1_counter + scale[2 * i + 1] * 1e-05 * velocity_y_2 / cam2_counter) + 1e5 * velocity[P_id + 2]) *
                (-5 * (scale[2 * i] * velocity_y_1 / cam1_counter + scale[2 * i + 1] * 1e-05 * velocity_y_2 / cam2_counter) + 1e5 * velocity[P_id + 2])) *
           OPTICAL_WEIGHT;
  }
  return 1;

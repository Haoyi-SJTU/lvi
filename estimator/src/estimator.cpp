...
void VIO_estimator::gyr_callback(const geometry_msgs::QuaternionStamped::ConstPtr &msg) //
{
  ...
  {
    gyr_init = 1;
    gyr.first = (float)(msg->header.stamp.sec) + (float)msg->header.stamp.nsec / 1000000000; // 时间戳
...
inline void VIO_estimator::publish_vio_result()
{
  ...
  // if (PUB_VIO_FLAG) // 如果产生了好数据
  {
    ...
    // if (!Ps_queue.empty())
    // {
    //   Eigen::Vector3d backVector = Ps_queue.back();
    //   result_Msg.transform.translation.x = backVector[0]; // 发布IMU预积分
    //   result_Msg.transform.translation.y = backVector[1];
    //   result_Msg.transform.translation.z = backVector[2];
